...
// ===== SURVEILLANCE BAN EN TEMPS RÉEL =====
let _banWatcher = null;

function startBanWatcher() {
  if (!window.FBDB || !currentUser) return;

  // Détache un ancien watcher si existe
  if (_banWatcher) {
    window.FBDB.ref(`bans/${currentUser.username}`).off('value', _banWatcher);
  }

  // Écoute en temps réel le nœud ban de l'utilisateur connecté
  _banWatcher = window.FBDB.ref(`bans/${currentUser.username}`).on('value', snap => {
    const ban = snap.val();
    if (ban) {
      // L'utilisateur vient d'être banni → affiche l'écran immédiatement
      sessionStorage.clear(); // Invalide la session
      showBanScreen(ban);
    }
  });
}
...
function stopBanWatcher() {
  if (_banWatcher && window.FBDB && currentUser) {
    window.FBDB.ref(`bans/${currentUser.username}`).off('value', _banWatcher);
    _banWatcher = null;
  }
}
...
function showBanScreen(ban) {
  ...
  document.getElementById('ban-date').textContent =
    `Banni le ${new Date(ban.bannedAt || Date.now()).toLocaleDateString('fr-FR', {day:'2-digit',month:'long',year:'numeric'})}`;
  document.getElementById('ban-by').textContent = `Par @${ban.bannedBy || 'administrateur'}`;

  // Génère les particules flottantes
  const container = document.getElementById('ban-particles');
...
function logout() {
  stopBanWatcher(); // Arrête la surveillance
  if (currentUser) {
...
function enterApp() {
  ...
  updateAdminAlertDot();
  startPolling();
  startBanWatcher(); // 🔴 Surveillance ban en temps réel
  switchTab('groups');
}
...
function openChat(type, id, pmPartner = null) {
  ...
  renderMessages(true); // Force scroll en bas à l'ouverture
...
function scrollToBottom() {
  ...
  if (!c) return;
  // Ne force le scroll en bas que si l'utilisateur est déjà en bas (± 150px)
...
function handleMsgScroll() {
  ...
  if (!c || !btn) return;
  // Affiche le bouton "remonter" si on a scrollé vers le bas
  const scrolledDown = c.scrollTop > 200;
...
Object.assign(window, {
  ...
  startBanWatcher, stopBanWatcher,
